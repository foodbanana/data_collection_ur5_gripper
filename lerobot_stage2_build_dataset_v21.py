...
#
# v3.0(0.6.1)용 원본에서 바뀐 곳은 딱 3군데:
#   1) add_frame: 'task'를 frame dict에서 빼서 인자로 넘김
#      (0.3.3의 add_frame(frame, task, timestamp=None) 시그니처. dict에 두면 ValueError)
#   2) finalize() 호출 삭제 (0.3.3엔 없음. save_episode()가 인코딩까지 끝냄)
#   3) 시작 시 CODEBASE_VERSION 검증 출력 (v2.1인지 즉시 확인)
#   ※ import 경로/create()/features/state·action 로직/save_episode()는 원본과 동일
#
...
def main():
    ...
    # ── (변경 3) v2.1 형식인지 즉시 확인 ──
    print(f"lerobot CODEBASE_VERSION = {CODEBASE_VERSION}")
    if CODEBASE_VERSION != 'v2.1':
        print(f"[경고] 이 환경의 lerobot은 v2.1이 아니라 '{CODEBASE_VERSION}' 형식을 만든다.")
        print("       v2.1을 원하면 lerobot 0.3.3 이 설치된 lerobot_v2 환경에서 실행하세요.")
        sys.exit(1)
    ...
    print(f"=== 2단계: LeRobotDataset 생성 (v2.1) ===")
    print(f"입력: {in_dir}")
    ...
    for i in range(N):
        ...
        # ── (변경 1) task 를 frame dict에서 빼서 인자로 넘김 ──
        dataset.add_frame(
            {
    ...
    # ── 에피소드 저장 (av1 mp4 인코딩까지 여기서 끝남) ──
    #   (변경 2) 0.3.3 엔 finalize() 가 없다. save_episode() 호출로 완결.
    ...
    print(f"  task: {args.task!r}")
    print(f"\n확인: python -c \"import json; "
          f"print(json.load(open('{os.path.join(root, 'meta', 'info.json')}'))['codebase_version'])\"")
    print("     -> v2.1 이 나와야 함")
